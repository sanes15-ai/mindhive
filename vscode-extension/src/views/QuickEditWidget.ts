...
export class QuickEditWidget {
  private activeSession?: EditSession;
  private statusBarItem: vscode.StatusBarItem;
  private undoStack: Array<{ ranges: vscode.Range[]; texts: string[] }> = [];
  
  private readonly commonSuggestions: QuickEditSuggestion[] = [
    {
  ...
  public async start() {
    ...
    if (!suggestion?.instruction) {
      ...
      instruction = await new Promise<string | undefined>((resolve) => {
  ...
  private buildContext(editor: vscode.TextEditor, range: vscode.Range): string {
  ...
  private buildEditPrompt(originalText: string, instruction: string, context: string): string {
...
Return ONLY the edited code:`;
  ...
  private extractEditedCode(generated: string, original: string): string {
    ...
    switch (choice.action) {
      case 'accept':
        ...
        vscode.window.showInformationMessage(
          `✅ Applied ${ranges.length} edit${ranges.length > 1 ? 's' : ''}`
        );
        this.dispose();
        break;
  ...
  private async reviewIndividualEdits(
    ranges: vscode.Range[],
    originalTexts: string[],
    editedTexts: string[]
  ) {
    const acceptedEdits: { range: vscode.Range; text: string }[] = [];
    ...
    for (let i = 0; i < ranges.length; i++) {
      ...
      if (choice.action === 'accept') {
        ...
        await this.showSideBySideDiff(
          [originalTexts[i]],
          [editedTexts[i]],
          this.activeSession!.editor.document.languageId
        );
    ...
    if (acceptedEdits.length > 0) {
      await this.applyEdits(
        acceptedEdits.map(e => e.range),
        acceptedEdits.map(e => e.text)
      );
      vscode.window.showInformationMessage(
        `✅ Applied ${acceptedEdits.length}/${ranges.length} edits`
      );
    }
  ...
  private async applyEdits(ranges: vscode.Range[], texts: string[]) {
    if (!this.activeSession) return;
