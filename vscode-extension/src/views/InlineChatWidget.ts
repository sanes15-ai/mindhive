...
export class InlineChatWidget {
  private activeSession?: InlineChatSession;
  ...
  private async processInstruction(instruction: string) {
  ...
  private parseInstruction(instruction: string): { type: string; context?: string } {
    const lower = instruction.toLowerCase();

    if (lower.includes('explain')) return { type: 'explain' };
    if (lower.includes('fix') || lower.includes('bug')) return { type: 'fix', context: instruction };
    ...
    if (lower.includes('test')) return { type: 'test' };
  ...
  private async fixCode(code: string, language: string, context?: string): Promise<string> {
  ...
  private async refactorCode(code: string, language: string, context?: string): Promise<string> {
    const prompt = context
  ...
  private async addDocumentation(code: string, language: string): Promise<string> {
    const response = await this.client.generateCode({
  ...
  private async customEdit(code: string, language: string, instruction: string): Promise<string> {
    const response = await this.client.generateCode({
      prompt: `${instruction}\n\nCode:\n${code}`,
      language,
      context: code,
  ...
  private async showDiffPreview(original: string, modified: string, range: vscode.Range) {
    if (!this.activeSession) return;
  ...
  private async showSideBySideDiff(original: string, modified: string, language: string) {
    ...
    await vscode.commands.executeCommand(
      ...
      originalDoc.uri,
      modifiedDoc.uri,
      'Original ↔ MindHive Suggestion'
    );
  }

  private async showExplanation(explanation: string) {
  ...
  private async showSecurityResults(verification: any) {
    ...
    if (verification.warnings.length > 0) {
      content += `## Warnings\n`;
      verification.warnings.forEach((warning: string) => {
        content += `- ${warning}\n`;
      });
    }
    ...
    if (verification.suggestions.length > 0) {
      content += `\n## Suggestions\n`;
      verification.suggestions.forEach((suggestion: string) => {
        content += `- ${suggestion}\n`;
      });
    }
  ...
  private async showTestsPreview(tests: string, language: string) {
    const doc = await vscode.workspace.openTextDocument({
      content: tests,
      language,
    });
    ...
    vscode.window.showInformationMessage(
      '✅ Tests generated! Review and save to your test file.',
      'Save to...'
    ).then((choice) => {
      if (choice === 'Save to...') {
