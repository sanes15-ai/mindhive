...
interface ErrorAnalysis {
  ...
  recommendations: Array<{
    ...
    successRate: number;
    fix: {
      type: 'code_change' | 'dependency' | 'configuration';
      changes: Array<{
        file: string;
        oldCode?: string;
        newCode: string;
...
export class TimeTravelDebugPanel {
  public static currentPanel: TimeTravelDebugPanel | undefined;
  private readonly _panel: vscode.WebviewPanel;
  private readonly _extensionUri: vscode.Uri;
  private _disposables: vscode.Disposable[] = [];
  private _currentAnalysis: ErrorAnalysis | null = null;

  constructor(
    panel: vscode.WebviewPanel,
    extensionUri: vscode.Uri,
    private readonly client: MindHiveClient
  ) {
    this._panel = panel;
  ...
  public static createOrShow(extensionUri: vscode.Uri, client: MindHiveClient) {
  ...
  public async analyzeError(error: Error, code: string, language: string) {
    try {
      // Show loading state
      ...
      if (similarCount > 0) {
        vscode.window.showInformationMessage(
          `🎯 Found ${similarCount} similar error${similarCount > 1 ? 's' : ''} with proven fixes!`
        );
      } else {
        vscode.window.showInformationMessage(
          '🆕 New error pattern detected. AI is generating a custom solution...'
        );
      }
    } catch (error: any) {
  ...
  private async applyFix(fixId: string) {
    ...
    const confirmation = await vscode.window.showWarningMessage(
      `Apply fix with ${(fix.confidenceScore * 100).toFixed(0)}% confidence?\n\n${fix.description}`,
      { modal: true },
      'Apply',
      'Preview',
      'Cancel'
    ...
    try {
      ...
      // Show success
      vscode.window.showInformationMessage(
        `✅ Fix applied successfully! Success rate: ${(fix.successRate * 100).toFixed(0)}%`
      );
  ...
  private async previewFix(fix: any) {
  ...
  private generatePreviewContent(fix: any): string {
    ...
    fix.preventiveMeasures.forEach((measure: string, index: number) => {
  ...
  private async showFixDetails(fixId: string) {
...
${fix.preventiveMeasures.map((m, i) => `${i + 1}. ${m}`).join('\n')}
  ...
  private async verifyFix(fixId: string) {
    if (!this._currentAnalysis) return;
