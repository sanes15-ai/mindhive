...
import { CursorPosition, SelectionRange } from './CollaborationEngine';
...
export interface Comment {
  ...
  createdAt: Date;
  editedAt?: Date;
  reactions: Map<string, number[]>; // emoji -> clientIds
  mentions: number[]; // clientIds mentioned
}
...
export class CommentThreadManager extends EventEmitter {
  private threads: Map<string, CommentThread> = new Map();
  private threadsByFile: Map<string, Set<string>> = new Map();
  ...
  createThread(
    filePath: string,
    range: SelectionRange,
    author: { clientId: number; name: string; email?: string },
   ...
   * Resolve thread
   */
  resolveThread(threadId: string, resolvedBy: number): void {
    const thread = this.threads.get(threadId);
    if (!thread) {
      throw new Error(`Thread ${threadId} not found`);
    }
   ...
   */
  addReaction(commentId: string, emoji: string, clientId: number): void {
    for (const thread of this.threads.values()) {
      const comment = thread.comments.find(c => c.id === commentId);
      if (comment) {
   ...
   */
  removeReaction(commentId: string, emoji: string, clientId: number): void {
    for (const thread of this.threads.values()) {
      const comment = thread.comments.find(c => c.id === commentId);
      if (comment) {
        const reactors = comment.reactions.get(emoji);
  ...
  filterThreads(filter: CommentFilter): CommentThread[] {
    ...
    if (filter.authorId !== undefined) {
      threads = threads.filter(t => 
        t.comments.some(c => c.author.clientId === filter.authorId)
      );
    }
   ...
   */
  getThreadsMentioning(clientId: number): CommentThread[] {
    return this.getAllThreads().filter(thread =>
      thread.comments.some(comment => comment.mentions.includes(clientId))
    );
  }
  ...
  getStatistics(): any {
    ...
    return {
      ...
      recentThreads: threads.filter(t => 
        Date.now() - t.updatedAt.getTime() < 24 * 60 * 60 * 1000
      ).length
    };
  }
  ...
  importThreads(data: any): void {
    ...
    for (const threadData of data.threads) {
      const thread: CommentThread = {
        ...
        comments: threadData.comments.map((c: any) => ({
          ...c,
