...
const VercelDeployer_1 = require("./VercelDeployer");
const NetlifyDeployer_1 = require("./NetlifyDeployer");
const RailwayDeployer_1 = require("./RailwayDeployer");
const BuildAutomation_1 = require("./BuildAutomation");
/**
...
class DeploymentManager {
    ...
    constructor(outputChannel) {
        ...
        this.vercelDeployer = new VercelDeployer_1.VercelDeployer(outputChannel);
        this.netlifyDeployer = new NetlifyDeployer_1.NetlifyDeployer(outputChannel);
        this.railwayDeployer = new RailwayDeployer_1.RailwayDeployer(outputChannel);
        this.buildAutomation = new BuildAutomation_1.BuildAutomation(outputChannel);
    ...
    async deploy(config) {
        ...
        try {
            ...
            if (result.success) {
                ...
                const action = await vscode.window.showInformationMessage(`🚀 Deployed to ${config.platform} in ${(duration / 1000).toFixed(1)}s`, 'Open Site', 'Copy URL', 'View Logs');
    ...
    async quickDeploy() {
        ...
        if (!token) {
            const action = await vscode.window.showWarningMessage(`No ${platform.platform} token found. Configure now?`, 'Configure', 'Cancel');
            if (action === 'Configure') {
    ...
    async showHistory() {
        ...
        const selected = await vscode.window.showQuickPick(this.deploymentHistory.map((deployment) => {
        ...
        if (selected && selected.deployment.url) {
            const action = await vscode.window.showInformationMessage(`Deployment to ${selected.deployment.platform}`, 'Open Site', 'Copy URL');
            if (action === 'Open Site') {
    ...
    async getToken(platform) {
        ...
        return config.get(`${platform}Token`);
    }
    /**
     ...
     */
    updateDeploymentStatus(status, message, progress) {
        if (this.currentDeployment) {
