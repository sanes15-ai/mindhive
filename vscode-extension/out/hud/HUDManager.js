...
class HUDManager {
    ...
    showFloatingHUD() {
        ...
        this.webviewPanel = vscode.window.createWebviewPanel('mindhiveHUD', '🧠 MindHive HUD', { viewColumn: vscode.ViewColumn.Beside, preserveFocus: true }, {
            enableScripts: true,
     ...
     */
    async fetchQualityMetrics(code, language) {
        try {
     ...
     */
    async fetchModelConsensus(code, language) {
        try {
    ...
    updateStatusBar(analytics) {
        ...
        this.statusBarItem.text = `$(brain) Hive | Quality: ${Math.round(quality)}% | Velocity: ${velocity}`;
    }
    async updateInlineDecorations(editor, analytics) {
        const decorations = [];
    ...
    async getFunctionInsight(line) {
        // Get insights from global patterns
        try {
            const patterns = await this.client.searchPatterns(line, { limit: 1 });
            if (patterns.length > 0) {
                const pattern = patterns[0];
                return `${pattern.usageCount.toLocaleString()} devs use similar | ${Math.round(pattern.successRate * 100)}% success`;
            }
        }
        catch (error) {
            // Silent fail
    ...
    async getPackageInsight(packageName) {
