...
class CollabPanel {
    ...
    panel;
    disposables = [];
    constructor(context, collaborationEngine, presenceManager, aiCollaborator, commentManager) {
    ...
    async show() {
        ...
        this.panel = vscode.window.createWebviewPanel('mindhiveCollaboration', '👥 Collaboration', vscode.ViewColumn.Beside, {
            enableScripts: true,
    ...
    setupEventListeners() {
        ...
        this.collaborationEngine.on('participant-joined', (participant) => {
            this.sendUpdate();
            this.showNotification(`${participant.name} joined the session`, 'info');
        });
        this.collaborationEngine.on('participant-left', (participant) => {
            this.sendUpdate();
    ...
    async handleStartSession(config) {
        try {
    ...
    async handleCreateComment(data) {
        try {
            ...
            this.commentManager.createThread(data.filePath, data.range, {
    ...
    async handleResolveComment(threadId) {
        try {
            const session = this.collaborationEngine.getCurrentSession();
            if (!session) {
                throw new Error('No active session');
    ...
    async handleAcceptAISuggestion(suggestionId) {
        try {
...
//# sourceMappingURL=CollabPanel.js.map