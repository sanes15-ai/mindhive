...
class ChatPanelProvider {
    ...
    client;
    authManager;
    static viewType = 'mindhive.chatView';
    _view;
    chatHistory = [];
    currentProjectMemoryId;
    constructor(_extensionUri, client, authManager) {
    ...
    async loadProjectChat() {
        try {
            const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
            if (!workspaceFolder)
                return;
            const projectMemory = await this.client.getOrCreateProjectMemory(workspaceFolder.uri.fsPath, vscode.workspace.name || 'Untitled');
            ...
            if (projectContext?.recentChat) {
                this.chatHistory = projectContext.recentChat.map((msg) => ({
                    role: msg.role,
                    content: msg.content,
    ...
    async handleUserMessage(message) {
        ...
        if (this.currentProjectMemoryId) {
            try {
                await this.client.addChatMessage(this.currentProjectMemoryId, 'user', message);
            }
            catch (error) {
        ...
        try {
            ...
            if (this.currentProjectMemoryId) {
                await this.client.addChatMessage(this.currentProjectMemoryId, 'assistant', response);
            }
        }
        catch (error) {
            this.chatHistory.push({
    ...
    async applyCodeToEditor(code, language) {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
