...
class TaskPlanner {
     ...
     */
    async planTasks(request, context) {
...
`.trim();
     ...
     */
    calculateEstimatedTime(tasks, dependencies) {
     ...
     */
    optimizeForParallelExecution(tasks, dependencies) {
        const levels = [];
     ...
     */
    async replanAfterFailure(originalPlan, failedTask, error) {
        const prompt = `
...
`.trim();
        ...
        try {
            const parsed = this.parseAIResponse(response.message);
            return this.planTasks(`Alternative approach: ${failedTask.description}`, `Previous attempt failed: ${error}`);
        }
        catch (error) {
     ...
     */
    estimateTaskTime(task, historicalData) {
    ...
    suggestOptimizations(plan) {
        ...
        if (plan.parallelizable && plan.parallelizable.length > 0) {
            const parallelCount = plan.parallelizable.reduce((sum, level) => sum + (level.length > 1 ? level.length - 1 : 0), 0);
            if (parallelCount > 0) {
